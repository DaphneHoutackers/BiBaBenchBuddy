...
import { ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Dna, FolderOpen, Plus, Play, RotateCcw, Save, Trash2 } from 'lucide-react';
...
const COPY = {
  en: {
    overview: 'Sequence overview', overviewHelp: 'Click a red marker to jump directly to that difference.', mismatch: 'Mismatch', alignment: 'Alignment', copyImage: 'Copy image', copied: 'Copied!', capturing: 'Capturing...',
    ...
    aligning: 'Aligning...', runAgain: 'Run again', start: 'Start Alignment', sequence: 'Sequence', fromLibrary: 'From library...', name: 'Name...', paste: 'Paste DNA sequence or FASTA...',
    enterBoth: 'Enter two sequences.', maximum: 'Maximum 10,000 bp per sequence.', identicalBases: 'Identical bases', identical: 'identical', insertions: 'Insertions (seq. 2)', deletions: 'Deletions (seq. 2)',
    ...
    savedAlignments: 'Saved alignments', noSaved: 'No saved alignments yet.', newAlignment: 'New alignment', save: 'Save', alignmentName: 'Alignment name', deleteAlignment: 'Delete alignment', deleteConfirm: 'Delete this saved alignment?', edited: 'Edited', collapsePanel: 'Collapse saved alignments', expandPanel: 'Expand saved alignments',
  },
  nl: {
    overview: 'Sequence-overzicht', overviewHelp: 'Klik op een rode marker om direct naar dat verschil te springen.', mismatch: 'Mismatch', alignment: 'Alignment', copyImage: 'Afbeelding kopiëren', copied: 'Gekopieerd!', capturing: 'Kopiëren...',
    ...
    aligning: 'Aligning...', runAgain: 'Opnieuw uitvoeren', start: 'Start Alignment', sequence: 'Sequentie', fromLibrary: 'Uit library...', name: 'Naam...', paste: 'Plak DNA-sequentie of FASTA...',
    enterBoth: 'Voer twee sequenties in.', maximum: 'Maximaal 10.000 bp per sequentie.', identicalBases: 'Identieke basen', identical: 'identiek', insertions: 'Inserties (seq. 2)', deletions: 'Deleties (seq. 2)',
    ...
    savedAlignments: 'Opgeslagen alignments', noSaved: 'Nog geen alignments opgeslagen.', newAlignment: 'Nieuwe alignment', save: 'Opslaan', alignmentName: 'Naam alignment', deleteAlignment: 'Alignment verwijderen', deleteConfirm: 'Deze opgeslagen alignment verwijderen?', edited: 'Bewerkt', collapsePanel: 'Opgeslagen alignments inklappen', expandPanel: 'Opgeslagen alignments uitklappen',
  },
};
...
function needlemanWunsch(s1, s2, match = 1, mismatch = -1, gap = -2) {
  ...
  for (let k = 0; k < aligned1.length; k++) {
    if (aligned1[k] === '-') { gaps++; insertions++; }
    else if (aligned2[k] === '-') { gaps++; deletions++; }
...
function FeatureTrack({ name, features, alignedSequence, length, t }) {
  const mapped = useMemo(() => mappedFeatures(features, alignedSequence), [features, alignedSequence]);
  return (
    <div className="relative z-10 grid grid-cols-[180px_1fr] items-center gap-3">
      ...
      <div className="relative h-9">
        ...
        {mapped.map(feature => {
          ...
          return <button key={feature.key} type="button" className="group absolute top-1/2 z-10 h-6 -translate-y-1/2 focus:outline-none hover:z-40 focus:z-40" style={{ left: `${left}%`, width: `${width}%`, minWidth: 6 }} aria-label={`${label}, ${Number(feature.start) + 1}–${feature.end}`}>
            <span className="flex h-full w-full items-center justify-center overflow-hidden border-2 border-white px-2 text-[10px] font-bold text-white shadow-sm" style={{ backgroundColor: feature.color || '#0f766e', clipPath: reverse ? 'polygon(8px 0,100% 0,100% 100%,8px 100%,0 50%)' : 'polygon(0 0,calc(100% - 8px) 0,100% 50%,calc(100% - 8px) 100%,0 100%)' }}>{width > 6 ? label : ''}</span>
            <span role="tooltip" className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 hidden min-w-44 -translate-x-1/2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-left shadow-xl group-hover:block group-focus:block">
...
function AlignmentOverview({ result, name1, name2, features1, features2, differences, viewport, onJump, t }) {
  const length = result.length || 1;
  const overviewRef = useRef(null);
  return (
    <section ref={overviewRef} className="rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-4 sm:px-5" aria-label="Alignment overview">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div><h4 className="text-sm font-bold text-slate-700">{t.overview}</h4><p className="text-[11px] text-slate-500">{t.overviewHelp}</p></div>
        <div className="flex items-center gap-3 text-[10px] text-slate-500"><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-red-500" />{t.mismatch}</span><CopyImageButton targetRef={overviewRef} label={t.copyImage} copiedLabel={t.copied} capturingLabel={t.capturing} data-html2canvas-ignore="true" className="h-7 gap-1.5 px-2 text-[10px]" /></div>
      </div>
      <div className="relative space-y-1">
        <div data-html2canvas-ignore="true" className="pointer-events-none absolute top-0 bottom-10 z-0 rounded-sm bg-slate-500/35 transition-[left,width] duration-100" style={{ left: `calc(192px + (100% - 192px) * ${viewport.start / length})`, width: `calc((100% - 192px) * ${(viewport.end - viewport.start) / length})` }} />
        <FeatureTrack name={name1} features={features1} alignedSequence={result.aligned1} length={length} t={t} />
        <FeatureTrack name={name2} features={features2} alignedSequence={result.aligned2} length={length} t={t} />
        <div className="relative z-10 grid grid-cols-[180px_1fr] items-center gap-3">
          <div className="text-right text-[10px] font-medium uppercase tracking-wide text-slate-400">{t.alignment}</div>
          <div className="relative h-9 rounded-md bg-white shadow-inner ring-1 ring-slate-200">
            <div data-html2canvas-ignore="true" className="pointer-events-none absolute inset-y-0 z-10 rounded-sm bg-slate-500/45 transition-[left,width] duration-100" style={{ left: `${(viewport.start / length) * 100}%`, width: `${((viewport.end - viewport.start) / length) * 100}%` }} />
            {differences.map((difference, index) => <button key={difference.index} type="button" onClick={() => onJump(index)} className="absolute inset-y-0 z-20 w-2 -translate-x-1/2 rounded-sm bg-red-500/80 transition hover:w-3 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-400" style={{ left: `${((difference.index + 0.5) / length) * 100}%` }} title={`${t.mismatch} ${index + 1}: ${difference.base1} → ${difference.base2}`} />)}
          </div>
        </div>
      </div>
    </section>
...
function SequenceViewer({ result, name1, name2, differences, activeDifference, onActiveDifference, viewport, onViewport, t }) {
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><div><h4 className="text-sm font-bold text-slate-700">{t.baseTitle}</h4><p className="text-[11px] text-slate-500">{t.baseHelp}</p></div><span className="rounded-md bg-slate-100 px-2 py-1 font-mono text-[11px] text-slate-600">bp {viewport.start + 1}–{Math.max(viewport.start + 1, viewport.end)}</span></div>
  ...
  if (collapsed) {
    return (
      <aside className="flex min-h-48 flex-col items-center border-l border-slate-200 bg-slate-50/70 py-3">
        ...
        <span className="mt-3 rounded-full bg-slate-200 px-1.5 py-0.5 text-[9px] font-bold text-slate-600">{items.length}</span>
      </aside>
    );
  }
  return (
    <aside className="min-h-48 overflow-hidden border-l border-slate-200 bg-slate-50/70 xl:sticky xl:top-3 xl:max-h-[calc(100vh-100px)]">
      <div className="flex items-center justify-between border-b border-slate-200 px-3 py-3">
        <div className="flex min-w-0 items-center gap-2"><button type="button" onClick={onToggle} className="flex h-7 w-7 flex-none items-center justify-center rounded-md text-slate-400 hover:bg-white hover:text-slate-700" aria-label={t.collapsePanel} title={t.collapsePanel}><ChevronRight className="h-4 w-4" /></button><div className="min-w-0"><h4 className="truncate text-xs font-bold text-slate-700">{t.savedAlignments}</h4><p className="text-[10px] text-slate-400">{items.length}</p></div></div>
...
export default function AlignmentView({ seq, seqName, features = [], library = [], language = 'en', storageScope = 'guest', draftId = 'default' }) {
  ...
  const [name1, setName1] = useState(initialDraft?.name1 ?? seqName ?? (language === 'nl' ? 'Sequentie 1' : 'Sequence 1'));
  const [name2, setName2] = useState(initialDraft?.name2 ?? (language === 'nl' ? 'Sequentie 2' : 'Sequence 2'));
  ...
  const [savedAlignments, setSavedAlignments] = useState(() => readStoredJson(libraryStorageKey, []));
  ...
  const persistSavedAlignments = next => {
    ...
    writeStoredJson(libraryStorageKey, next);
  };
  const saveAlignment = () => {
    if (!result) return;
  ...
  const startNewAlignment = () => {
    setSeq1(seq || ''); setSeq2(''); setName1(seqName || (language === 'nl' ? 'Sequentie 1' : 'Sequence 1')); setName2(language === 'nl' ? 'Sequentie 2' : 'Sequence 2');
  ...
  return (
    <div className={`grid min-h-0 transition-[grid-template-columns] duration-200 ${savedPanelCollapsed ? 'md:grid-cols-[minmax(0,1fr)_44px]' : 'md:grid-cols-[minmax(0,1fr)_280px]'}`}>
      ...
      <div className={`mr-0 grid overflow-hidden transition-[max-height,opacity] duration-300 md:mr-4 md:grid-cols-2 md:gap-3 ${inputCollapsed ? 'max-h-0 opacity-0' : 'max-h-[650px] opacity-100'}`} aria-hidden={inputCollapsed}>
        {[{ number: 1, name: name1, setName: setName1, sequence: seq1, setSequence: setSeq1 }, { number: 2, name: name2, setName: setName2, sequence: seq2, setSequence: setSeq2 }].map(field => <div key={field.number} className="pb-1"><label className="mb-1 block text-xs font-medium text-slate-600">{t.sequence} {field.number} <span className="text-slate-400">({cleanSequence(field.sequence).length} bp)</span></label><div className="mb-1 flex gap-1"><input value={field.name} onChange={event => field.setName(event.target.value)} placeholder={t.name} className="min-w-0 flex-1 rounded-md border border-slate-200 px-2 py-1 text-xs" />{library.length > 0 && <select className="w-28 truncate rounded-md border border-slate-200 bg-white px-1 py-1 text-xs text-slate-600" onChange={event => { selectLibraryEntry(library.find(item => item.id === event.target.value), field.number); event.target.value = ''; }} defaultValue=""><option value="" disabled>{t.fromLibrary}</option>{library.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select>}</div><Textarea value={field.sequence} onChange={event => { field.setSequence(event.target.value); setResult(null); }} placeholder={t.paste} className="h-[260px] min-h-[180px] resize-y border-slate-200 bg-white font-mono text-xs leading-relaxed" /></div>)}
      </div>
      {error && <p className="mr-0 text-xs text-red-500 md:mr-4">{error}</p>}
      {result && <div className="mr-0 space-y-4 md:mr-4"><div className="flex flex-wrap items-stretch divide-x divide-slate-200 overflow-hidden rounded-lg border border-slate-200 bg-slate-50/70 text-slate-700"><div className="min-w-52 flex-1 px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t.identicalBases}</p><div className="mt-0.5 flex items-baseline gap-2"><strong className="text-sm font-bold text-slate-800">{result.matches.toLocaleString()} / {result.length.toLocaleString()} bp</strong><span className="text-xs font-semibold text-slate-500">{result.identity}% {t.identical}</span></div></div>{[[t.mismatch, result.mismatches], [t.insertions, result.insertions], [t.deletions, result.deletions]].map(([label, value]) => <div key={label} className="min-w-28 px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-0.5 text-sm font-bold text-slate-800">{value.toLocaleString()}</p></div>)}</div><AlignmentOverview result={result} name1={name1} name2={name2} features1={features1} features2={features2} differences={differences} viewport={viewport} onJump={jumpToDifference} t={t} /><SequenceViewer result={result} name1={name1} name2={name2} differences={differences} activeDifference={activeDifference} onActiveDifference={setActiveDifference} viewport={viewport} onViewport={setViewport} t={t} /></div>}
